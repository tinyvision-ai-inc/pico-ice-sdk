...
int main() {
    ...
    for (;;) {
        ice_usb_task();
        ice_ssram_write(ICE_SSRAM_SPI_CS_PIN, START_ADDR, write_data, sizeof(write_data), true /* async */);
        ice_ssram_read(ICE_SSRAM_SPI_CS_PIN, read_data, START_ADDR, sizeof(read_data), true /* async */);
