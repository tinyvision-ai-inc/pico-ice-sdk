...
void ice_spi_read_blocking(uint8_t csn_pin, uint8_t const *buf_r, size_t len)
{
    ice_spi_write_async(csn_pin, buf_r, len, NULL, NULL);
    ice_spi_await_async_completion();
}

void ice_spi_write_blocking(uint8_t csn_pin, uint8_t *buf_w, size_t len)
{
    ice_spi_write_async(csn_pin, buf_w, len, NULL, NULL);
    ice_spi_await_async_completion();
}
