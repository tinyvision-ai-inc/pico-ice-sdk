#include "ice_fpga_bitstream.pio.h"
...
static bool try_add_program(PIO try_pio) {
    ...
    pio = try_pio;
    offset = pio_add_program(pio, &ice_fpga_bitstream_program);
...
static void state_machine_init() {
    ...
    pio_sm_config c = ice_fpga_bitstream_program_get_default_config(offset);
...
static void state_machine_deinit() {
    ...
    pio_remove_program(pio, &ice_fpga_bitstream_program, offset);
...
bool ice_fpga_bitstream_write(const uint8_t* bitstream, uint32_t size) {
